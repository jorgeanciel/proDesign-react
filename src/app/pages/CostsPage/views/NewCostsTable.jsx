...
export default function NewCostsTables({
	project,
	costs,
	handleCosts,
	numberOfClassrooms,
...
function VersionTable({
	...
	// const areaMt2Secundaria = VAR_PRECIO_MT2 * numberOfClassrooms.secundaria;
	// const areaMt2Biblioteca =
	...
	const rows = [
		createData(
			"Costo total de la estructura del proyecto",
			`S/${formatNumber(total_structure)}`
		),
		createData(
			"Costo total del equipamiento",
			`S/${formatNumber(total_finishes)}`
		),
		createData(
			"Costo total de las instalaciones eléctricas y sanitarias",
			`S/${formatNumber(total_installations)}`
		),
