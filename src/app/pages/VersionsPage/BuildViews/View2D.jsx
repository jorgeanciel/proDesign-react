...
export default function View2D({ selectedVersion }) {
	// Plan 3D all data
	let { result_data, classroom_measurements, construction_info } = JSON.parse(
		selectedVersion.build_data
	);
	...
	let data = {
		levels: JSON.parse(selectedVersion.level).map((level) =>
			level.toLowerCase()
		),
		aforo: {
	...
	// let baths_amount = {}
	let total_bathrooms =
		amount_bathrooms.inicial +
		amount_bathrooms.primaria +
		amount_bathrooms.secundaria;

	if (data.levels.includes("inicial")) {
		let pab1_baths =
			amount_bathrooms.inicial <= 6 ? amount_bathrooms.inicial : 6;
		baths_amount.push({
			pab: 1,
			baths: pab1_baths,
		});
		...
		let pab2_baths =
			amount_bathrooms.primaria + amount_bathrooms.secundaria <= 6
				? amount_bathrooms.primaria + amount_bathrooms.secundaria
				: 6;
		baths_amount.push({
			pab: 2,
			baths: pab2_baths,
	...
	let classroom = {
		length:
			classroom_measurements.muro_horizontal * increment_scale +
			wall_thickness * 2,
		width:
			classroom_measurements.muro_vertical * increment_scale +
			wall_thickness * 2,
		height: 2.2 * increment_scale,
		bigas: {
			horizontal: {
				width: 0.3 * increment_scale,
				height: 0.6 * increment_scale,
			},
			vertical: {
				width: 0.3 * increment_scale,
				height: 0.4 * increment_scale,
			},
		},
	};

	let bathroom = {
		lavamanos: 0.6 * increment_scale,
		inodoro: 1.4 * increment_scale, // largo del cubiculo
		ancho_de_cubiculo: 0.85 * increment_scale, // ancho del baño (cubiculo y lavamanos)
		pasillo: 1.2 * increment_scale,
	...
	return (
		<Canvas
			...
			camera={{
				position: [
					0.0033158862420857344, 3315.741671368019,
					0.000004318159471182016,
				],
				rotation: [
					"-1.5707963254925756",
					"0.000001000043601318168",
					"1.5694940634737558",
					"XYZ",
				],
				manual: true,
		...
		>
			...
			<Terrain2D width={terrain.width} length={terrain.length} />
		</Canvas>
	);
}
