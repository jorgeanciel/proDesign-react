...
export default function Pabellon_({
	position,
	vista,
	rotation,
	classroom,
	...
	const stairsOffset = (x) => {
		return n_pabellon === 1
			? x - (classroom.length - 120)
			: x + (classroom.length - 120);
	};

	const bathroomOffset = (x) => {
		return n_pabellon === 1
			? x - (classroom.length - 334.9999938979745)
			: x + (classroom.length - 334.9999938979745);
	};
	...
	const buildPeine = (classrooms_for_peine, floor) => {
		...
		for (let i = 0; i < classrooms_for_peine.length; i++) {
			oRoom.push({
				...
				rotation_classroom: [
					0,
					MathUtils.degToRad(side === "top" ? 90 : -90),
					0,
				],
	...
	return (
		<group position={position} ref={pabellon_group}>
			{oRoom.map((el, index) => (
				<el.room
					key={index}
