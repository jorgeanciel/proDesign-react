...
export default function Pabellones_({
	amount_classrooms,
	vista,
	classroom,
	bathroom,
	...
	let total_classrooms_length =
		(amount_classrooms + 1) * classroom.length + 334.9999938979745; // 334.9999938979745 is bathroom length
	let amount_pabellones = total_classrooms_length < terrain.width ? 1 : 2;

	let pab = {
		1: {
			// (classroom.length * 2) es el offset en la parte superior del pabellon.
		...
		2: {
			// (classroom.length) es el offset en la parte superior del pabellon. NOTE: No se multiplica x2 porque este pabellon no ha sido rotado como el primero.
			...
			max_classrooms: computeMaxClassrooms({
				have_bathroom:
					baths_amount.find((el) => el.pab === 2).baths > 0,
				have_stairs: false,
			}),
			floors: [],
		},
	...
	let max_classrooms_peine = Math.floor(
		(terrain.width - 2 * (pasillo + classroom.width)) / classroom.length
	); // +1 (forzar un aula más en peine)
	// Formula: (MaxClassroomsP1 + MaxClassroomsP2 + 3 * MaxClassroomsPeine) - 1
	let max_classrooms_for_floor =
		pab[1].max_classrooms +
		pab[2].max_classrooms +
		3 * max_classrooms_peine -
		1;
	...
	if (amount_classrooms > max_classrooms_for_floor) {
		pab[1].max_classrooms = computeMaxClassrooms({
			have_bathroom: true,
			have_stairs: true,
		});
		if (max_classrooms_for_floor + pab[1].max_classrooms) {
			pab[2].max_classrooms = computeMaxClassrooms({
				have_bathroom:
					baths_amount.find((el) => el.pab === 2).baths > 0,
				have_stairs: true,
			});
	...
	console.log({
		max_classrooms_peine,
		max_classroom_first_floor: max_classrooms_for_floor,
	});
	console.log({
		"max classrooms first pabellon:": pab[1].max_classrooms,
		"max classrooms second pabellon:": pab[2].max_classrooms,
	});
	...
	if (amount_pabellones === 1) {
		...
		pab[2].floors[0].classrooms_for_peine = getClassrooms(
			classrooms,
			classrooms_for_peine
		);
		...
		if (pab[2].floors[1])
			pab[2].floors[1].classrooms_for_peine = getClassrooms(
				classrooms,
				classrooms_for_peine2
			);
		...
		if (pab[2].floors[2])
			pab[2].floors[2].classrooms_for_peine = getClassrooms(
				classrooms,
				classrooms_for_peine3
			);
