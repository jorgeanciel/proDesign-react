		...
		<Paper sx={{ width: "100%" }}>
			...
			<TableContainer sx={{ overflowY: "hidden" }}>
				...
				<Table sx={{ borderCollapse: "collapse" }}>
					...
					<TableBody
						sx={{ ".MuiTableCell-root": { fontSize: 12.5 } }}
					>
						{parentProjects
							?.slice(
								page * rowsPerPage,
							...
							.map((row, i) => (
								<Row
									key={row.id}
									row={row}
									versions={projects.filter(
										(el) => row.id === el.parent_id
									)}
									color={
										i % 2 === 0 ? "#0000000a" : undefined
									}
									expand={expand}
								/>
							))}
	...
	const handleDelete = (id, parentID, name) => () => {
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				Swal.fire("Exito", "El proyecto ha sido eliminado", "success");
			}
		});
	};
	...
	return (
		<>
			...
			>
				...
				<TableCell>
					{new Date(row.updatedAt).toLocaleString("es-ES", {
						weekday: "short",
						year: "numeric",
						month: "long",
						day: "2-digit",
...
const CarruselRow = ({ open, version, handleDelete }) => {
	return (
		...
		>
			...
			<TableCell>{version.client}</TableCell>
			<TableCell>Privado</TableCell>
			<TableCell>
				{open && (
					<Box sx={{ display: "flex", justifyContent: "center" }}>
