...
import {
	WALL_THICKNESS,
	INCREMENT_SCALE as increment_scale,
} from "./app.settings";
...
export class Bathroom {
	...
	constructor() {
		...
		this.handwash = 0.6 * increment_scale;
		this.toilet = 1.4 * increment_scale; // largo del cubiculo
		this.cubicleWidth = 2.85 * increment_scale; // ancho del baño (cubiculo y lavamanos)
		this.corridor = 1.2 * increment_scale;
		this.entranceCorridor = 1 * increment_scale;

		this.walls = new Walls(this);
	}

	set _handwash(value: number) {
...
class Walls {
	...
	setGeometry(width: number) {
		const shape = createWallShape(this._length, width, WALL_THICKNESS);
...
class Entrance {
	...
	constructor(bathroom: Bathroom) {
		this.position = [
			bathroom.length / 2 - bathroom.entranceCorridor / 2,
			0,
			0,
		]; // z = 220 - 7.5 == 212.5 (el position[2] se cambia automaticamente en bathroom.walls.setCubicles())
